...
src/                  ES Module 源码（开发用）
  ...
  cinema.js           电影院场景：圆筒环墙影厅、可隐藏出口、多路出声控制台、片单历史
...
想回到球场，点放映条最右的「**🚪 退出影院**」，也可以走「出口」红门。
放映条点「**隐藏**」就整个收起来（观影不被按钮挡），左下角留一个「**☰ 放映条**」小钮点一下唤回。

### 🚪 隐藏出口 = 纯放映模式

控制台里点「**🚪 隐藏出口**」：厅壳缺掉的那一角被补成**完整 360°**，红门消失，
**银幕槽位随之吃满一整圈**（例如 2 部片从 170°/170° 变成 180°/180°），黑匣子观影不留门洞。
隐藏后红门不再参与射线检测（THREE 的 Raycaster **不会跳过 `visible:false` 对象**，
若不从检测列表里剔除，隐形的门会挡住身后的床与银幕），退出只能走放映条的「🚪 退出影院」。
这个开关本身也记住（`bb.cinema.door`）。
...
| 底部「**↺ 恢复默认**」 | 退回 `video/` 片单 |
| 底部「**🚪 隐藏出口**」 | 门洞补成完整黑匣子，银幕槽位吃满 360°（再点还原） |
| 底部「**⏳ 播放过**」 | 历史片单快照，见下 |
