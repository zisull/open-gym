...
**滚轮拉近拉远**（变焦只改视野，起身自动复原），底部弹出放映条（暂停/播放、音量、
放大观看、选择视频、恢复默认、隐藏、起身）。右键或 WASD 起身，走「出口」红门可回到球场。
点任意屏幕 = 把声音切给那块屏并切换它的播放/暂停（多路同时放，只有一路出声，避免混音）。
放映条点「**隐藏**」就整个收起来（观影不被按钮挡），左下角留一个「**☰ 放映条**」小钮点一下唤回。

**「放大观看」= 片源管理墙**：宫格铺满整屏，每格右上角有 **✕**（把这部片从环上删掉，银幕一起
少一块），末尾还有一格 **＋ 加入视频**（**追加**，与放映条「选择视频」的整条替换互补）。删/加之后
环的排布和存档同步更新。

**加片只有两条入口**：放映条上点「选择视频」，**可以多选，多选即整条替换片单**——
这次选了几部，环上就只放这几部，之前的片源不再保留；上限 12 块屏，多出的忽略。
「恢复默认」把环上的排布退回 `video/` 片单。银幕顺序、各片宽高比与音量都会存进
`localStorage`，下次打开
