...
影厅是一个**圆筒形黑匣子**，环墙上排着弧形银幕——**银幕数量 = 片源数量**，片单里有
3 部就 3 块屏、5 部就 5 块屏，在整圈圆弧上均匀分布。**所有银幕等高**，每块的弧长按
自己视频的宽高比换算（横屏占的圆心角大、竖屏占的小），所以横屏竖屏都不会被裁或被拉伸。
影厅中央是一张 360° 环形大沙发，**站到沙发边左键即入座**（任意位置、任意朝向），
入座后按住左键拖拽可自由环视环墙，底部弹出放映条（暂停/播放、音量、放大观看、
选择视频、恢复默认、起身）。右键或 WASD 起身，走「出口」红门可回到球场。
点任意屏幕 = 把声音切给那块屏并切换它的播放/暂停（多路同时放，只有一路出声，避免混音）。

**加片只有一条规则**：放映条上点「选择视频」，**可以多选**——列表里没有的同名片源
就追加为新银幕（排到环尾），已有的同名片源就地替换那块屏；上限 12 块。「恢复默认」
把环上的排布退回 `video/` 片单。银幕顺序与音量都会存进 `localStorage`，下次打开
