...
四面墙各一块大银幕，**同时循环放映**（默认给片单前 4 个视频各分一块屏），每块屏
按自己视频的宽高比在墙框内取最大矩形、黑框跟着画面收放（横屏、竖屏、方屏都原比例
显示）；影厅中央是一张 360° 环形大沙发，**站到沙发边左键即入座**（任意位置、任意
朝向），入座后按住左键拖拽可自由环视四面墙，底部弹出放映条（暂停/播放、音量、
**四宫格大屏**、选择视频、起身）。右键或 WASD 起身，走「出口」红门可回到球场。
点任意屏幕 = 把声音切给那块屏并切换它的播放/暂停（四路同时放，只有一路出声，避免混音）。

**换片只有一条规则**：面朝哪面墙，点「选择视频」就只替换**那块**银幕。每块屏放的是
哪部、以及音量，都会存进 `localStorage`，下次打开 `index.html` 自动复原成上次的排布。
