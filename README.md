...
- **自由模式**：无限时，拍球分 + 投篮分合并计总分。持球时左键即拍球，
  自动跟手无任何判定难度，纯属氛围与加分（每次 +2）。
- **投篮限时挑战（90s）**：开局空投到随机投篮点、球已在手。持球走进三分线投篮区
  自动切入瞄准视角，力度条绿段为"最佳力度"（按抛物线反解实时计算），
  松开左键出手、右键假投。进球有粒子+震屏+刷网音+泛光脉冲；
  **每进一球立即随机传送到新的投篮点**（防止蹲守单一位置练出肌肉记忆），
  未中则原地再来；连续 3 不中连击清零；球落地自动回手。
- 三分线外命中 30 分基础、线内 20 分，均乘投篮连击倍数（1/1/2/3 封顶）。
