...
export const CFG = {
  ...
  shot: {
    ...
    spotArc: 1.40,     // 允许偏离半场正面的最大角度（±80°）：两侧再就贴着篮板平行线了
  ...
  arch: {
    ...
    targetY: 1.6, targetZ: 12.72, faceR: 0.75, boardHalf: 0.92,
    ...
    speedMin: 9,
    speedMax: 75,
    stepT: 1 / 240,      // 定步长积分：与命中平面的插值求解配套，步长再大也不漏判
  ...
  cinema: {
    ...
    gymDoor: { x: 6.0, z: 16.35, r: 1.25, hint: 3.6 }, // r=走进即传送，hint=还差几步就开始提示
  ...
  pool: {
    gymDoor: { x: -6.0, z: 16.35, r: 1.25, hint: 3.6 }, // 球馆侧入口（与电影院红门左右对称）
    ...
    duel: {
      ...
      next: 4.5,                // 一局打完，结果亮这么久就自动重摆开下一局（开球方轮换）
